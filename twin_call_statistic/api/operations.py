...
from twin_call_statistic.api.crud import (
    get_twin_accounts, get_from_date, save_contacts
)
...
async def receive_contacts_info(request: Request):
    async with get_session(request) as session:
        ...
        if not accounts:
            raise HTTPException(status_code=404, detail="Список аккаунтов для сбора пуст")

        accounts = [{"login": acc.twin_login, "pass": acc.twin_password} for acc in accounts]
        for account in accounts:
            login, password = account["login"], account["pass"]
            twin = await get_twin_repo(request, login, password)
            token = await twin.get_auth_token()
            from_date = await get_from_date(session, login)
            from_date = from_date if from_date else request.app.container.settings.date_start
            await save_contacts(
                twin=twin, token=token, project=login, session=session, from_=from_date
            )
